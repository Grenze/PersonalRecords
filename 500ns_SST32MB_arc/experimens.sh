...
do
#echo $db
testAndMkdir $value_size_results$db $large_dataset$db $snapshot$db $ycsb$db $readwritemix$db
done
...
exe_file_ycsb="/home/"${user_name}"/benchmark/ycsbc"
...
do
echo ${exe_file_ycsb}" -db "${db}" -dbfilename "${seat}${db}" -threads "${th}" -P "${workloads_dir}${workload}
exe_str=${exe_file_ycsb}" -db "${db}" -dbfilename "${seat}${db}" -threads "${th}" -P "${workloads_dir}${workload}
nohup $exe_str >> $output &
wait $!
rm -rf ${seat}${db}
#echo ${seat}${db}
...
num=$((data_size/vs))
#echo $num
output=${readwritemix}${db}"/"${db}"_value_size_"${vs}
#echo $output
testAndTouch $output
if [ $? -eq 1 ]; then
echo ${parent_path}${db}${exe_file}" --threads="${th}" --write_threads="${write_th}" --value_size="${vs}" --num="$num" --db="${seat}${db}" --benchmarks="${db_bench}${plus}
exe_str=${parent_path}${db}${exe_file}" --threads="${th}" --write_threads="${write_th}" --value_size="${vs}" --num="$num" --db="${seat}${db}" --benchmarks="${db_bench}${plus}
nohup $exe_str >> $output &
wait $!
rm -rf ${seat}${db}
#echo ${seat}${db}
